...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    setLoading(true);
    const id = toast.loading("Authenticating session...");
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast.error("Google login failed.", { id });
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="relative flex items-center justify-center min-h-[calc(100vh-4rem)] p-6 z-10 w-full max-w-md mx-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2">Log in to manage your growth pipeline.</p>
        </div>
        ...
        <div className="glass-card p-8 sm:p-10">
          ...
          <button onClick={handleGoogleLogin} disabled={loading} className="btn-secondary w-full flex gap-3 text-sm">
