...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    if (password !== confirmPassword) {
      toast.error("Access tokens must match.");
      return;
    }
    
    setLoading(true);
    const loadingToast = toast.loading("Initializing new agent...");
    try {
      await createUserWithEmailAndPassword(auth, email, password);
      toast.success("Agent successfully deployed!", { id: loadingToast });
      router.push("/blogs");
    } catch (error) {
      console.error("Signup error:", error);
      toast.error(error.message || "Deployment failed.", { id: loadingToast });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignup = async () => {
    setLoading(true);
    const loadingToast = toast.loading("Connecting to Global Auth...");
    try {
      await signInWithPopup(auth, googleProvider);
      toast.success("Auth bridge established!", { id: loadingToast });
      router.push("/blogs");
    } catch (error) {
      console.error("Google signup error:", error);
      toast.error("Auth bridge failure.", { id: loadingToast });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="glass-card p-10 bg-slate-900/40 backdrop-blur-3xl ring-1 ring-white/5 shadow-2xl">
          <form onSubmit={handleSignup} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Universal Email</label>
              <input
                type="email"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Verify Token</label>
              <input
                type="password"
                placeholder="••••••••"
                className="input-field"
          ...
          <div className="relative my-10">
            ...
            <div className="relative flex justify-center text-[10px] font-black uppercase tracking-widest">
              <span className="bg-[#0b1120] px-4 text-slate-600">Global Auth Bridge</span>
            </div>
          </div>
          ...
          <p className="mt-10 text-center text-sm text-slate-500 font-medium tracking-tight">
            Already initialized?{" "}
            <Link href="/login" className="text-brand-400 font-black hover:text-brand-300 transition-colors uppercase text-xs tracking-[0.2em]">
              Access Core
            </Link>
          </p>
        </div>
